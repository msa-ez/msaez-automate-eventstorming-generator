...
class FirebaseSystem(DatabaseSystem):
    ...
    def watch_data(self, path: str, callback: Callable[[Optional[Dict[str, Any]]], None]) -> bool:
        ...
        def _watch_operation():
            ...
            def listener(snapshot):
                try:
                    ...
                    if data is not None and isinstance(data, dict):
                        callback(self.restore_data_from_firebase(data))
            ...
            listener_registration = ref.listen(listener)
            self._listeners[path] = listener_registration
            return True
    ...
    def unwatch_data(self, path: str) -> bool:
        ...
        def _unwatch_operation():
            if path in self._listeners:
                listener_registration = self._listeners[path]
                # SSE 연결 완전 종료 (close() 메서드 사용)
    ...
    def restore_data_from_firebase(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        return {k: self.restore_value(v) for k, v in data.items()}
