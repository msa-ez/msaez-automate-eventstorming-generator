...
from ....commons.graph import execute_create_command_actions_by_function_sub_graph
from .....constants import RG

def run_create_command_actions_by_function_sub_graph(command_args):
    run_name = "run_create_command_actions_by_function_sub_graph"
    after_stop_node = RG.CREATE_COMMAND_ACTIONS

    try:
        db_type = ListUtil.get_safely(command_args, 2, "memory")
        Config.set_db_type(db_type)

        state: State = execute_create_command_actions_by_function_sub_graph()
