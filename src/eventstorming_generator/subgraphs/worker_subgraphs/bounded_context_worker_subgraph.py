...
from ...models import BoundedContextGenerationState, State, CreateBoundedContextGeneratorOutput
from ...utils import JsonUtil, LogUtil
from ...generators import CreateBoundedContextGenerator
from ...config import Config

# 스레드로부터 안전한 컨텍스트 변수 생성
bounded_context_worker_id_context = ContextVar('worker_id', default=None)

def get_current_generation(state: State) -> Optional[BoundedContextGenerationState]:
    """
    ...
    worker_id = bounded_context_worker_id_context.get()  # 공유 상태가 아닌 컨텍스트 변수에서 ID를 가져옴
    
    if not worker_id:
        LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] Current worker ID not found in state")
        return None
    
    if worker_id not in model.worker_generations:
        LogUtil.add_error_log(state, f"[BOUNDED_CONTEXT_WORKER] Worker generation not found for worker_id: {worker_id}")
        return None
    
    return model.worker_generations[worker_id]

def worker_preprocess_bounded_context(state: State) -> State:
    """
    단일 요구사항 청크 전처리 (워커 전용)
    """
    current_gen = get_current_generation(state)
    if not current_gen:
        LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] No current generation found in worker preprocess")
        return state
...
def worker_generate_bounded_context(state: State) -> State:
    """
    단일 요구사항 청크 Bounded Context 생성 (워커 전용)
    """
    current_gen = get_current_generation(state)
    if not current_gen:
        LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] No current generation found in worker generate")
        return state
    ...
    try:
        ...
        generator = CreateBoundedContextGenerator(
...
def worker_postprocess_bounded_context(state: State) -> State:
    """
    단일 요구사항 청크 Bounded Context 후처리 (워커 전용)
    """
    current_gen = get_current_generation(state)
    if not current_gen:
        LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] No current generation found in worker postprocess")
        return state
        
    
    try:
        # 생성된 Bounded Contexts가 없으면 실패로 처리
        if not current_gen.created_bounded_contexts or len(current_gen.created_bounded_contexts) == 0:
            current_gen.retry_count += 1
            return state
        ...
        if current_gen:
            current_gen.retry_count += 1
            current_gen.created_bounded_contexts = []

    return state

def worker_validate_bounded_context(state: State) -> State:
    """
    단일 요구사항 청크 Bounded Context 검증 (워커 전용)
    - 생성 완료 및 재시도 횟수 확인
    """
    current_gen = get_current_generation(state)
    if not current_gen:
        LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] No current generation found in worker validate")
        return state
        
    try:
        # 최대 재시도 횟수 초과 시 실패로 처리
        if current_gen.retry_count > state.subgraphs.createBoundedContextByFunctionsModel.max_retry_count:
...
def worker_decide_next_step(state: State) -> str:
    ...
    try:
        ...
        if not current_gen:
            LogUtil.add_error_log(state, "[BOUNDED_CONTEXT_WORKER] No current generation found in decide_next_step")
            return "complete"

        # 실패 혹은 최대 재시도 횟수 초과 시 완료
        if current_gen.is_failed or current_gen.retry_count > state.subgraphs.createBoundedContextByFunctionsModel.max_retry_count:
            return "complete"
...
def create_bounded_context_worker_subgraph():
    """
    단일 Bounded Context 처리를 위한 워커 서브그래프 생성
    ...
    # 노드 추가
    worker_graph.add_node("preprocess", worker_preprocess_bounded_context)
    worker_graph.add_node("generate", worker_generate_bounded_context) 
    worker_graph.add_node("postprocess", worker_postprocess_bounded_context)
    worker_graph.add_node("validate", worker_validate_bounded_context)
    ...
    def run_worker(state: State) -> State:
        ...
        Returns:
            State: 처리 완료된 boundedContexts를 포함한 State
        """
        try:
