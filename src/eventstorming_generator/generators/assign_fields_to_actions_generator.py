...
class AssignFieldsToActionsGenerator(XmlBaseGenerator):
    ...
    <core_instructions>
        ...
        <guidelines>
            ...
            <rule id="9">**Source Reference Format:** The format for `refs` is an array of Position Arrays: `[[["<start_line_number>", "<start_word_combination>"], ["<end_line_number>", "<end_word_combination>"]]]`.

                **WHEN refs is REQUIRED vs OPTIONAL:**
                - **REQUIRED (non-empty):** If this field exists because requirements specifically mentioned the attribute / concept (e.g., text says "주문 일자, 주문자 이름" → fields like `order_date`, `customer_name` MUST have refs).
                - **OPTIONAL (empty `[]`):** Standard DDD/audit fields (`createdAt`, `updatedAt`, `version`, `id`) added for completeness without specific mention can return `"refs": []` HONESTLY.
                - **NEVER fabricate refs.** Empty = honest "standard pattern"; fabricated = lie.

                **Phrase Selection (when non-empty):** 2-4 token phrases at CLAUSE boundaries, never mid-word. Skip headers (`#`..`######`), table rows (`|`), separator (`---`), blank lines — anchor only on prose. Per-field specific. Zero-length forbidden. Tight spans (≤3 lines).</rule>
